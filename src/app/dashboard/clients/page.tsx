    ...
    <div className="p-8">
      <div className="flex items-end justify-between mb-10">
        <div>
          ...
          <h1 className="font-display text-4xl font-light italic text-[#0a0a0a]">Clients</h1>
        </div>
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-[#0a0a0a]/5">
              {['Nom', 'Email', 'Téléphone', 'Commandes', 'Total dépensé', 'Actions'].map(h => (
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={6} className="px-6 py-10 text-center font-mono text-[0.6rem] text-[#8a8580] tracking-widest">Aucun client</td></tr>
            ) : clients.map(c => (
              <tr key={c.id} className="border-b border-[#0a0a0a]/5 hover:bg-[#f5f2ee] transition-colors">
                <td className="px-6 py-4 text-sm text-[#0a0a0a] font-medium">{c.nom}</td>
                <td className="px-6 py-4 font-mono text-[0.65rem] text-[#8a8580]">{c.email}</td>
                <td className="px-6 py-4 font-mono text-[0.65rem] text-[#8a8580]">{c.telephone || '—'}</td>
                <td className="px-6 py-4 font-mono text-sm text-[#0a0a0a]">{c.total_commandes}</td>
                <td className="px-6 py-4 font-mono text-sm text-[#0a0a0a]">{c.total_depense}€</td>
                <td className="px-6 py-4">
                  <button onClick={() => supprimer(c.id)} className="font-mono text-[0.55rem] tracking-widest uppercase text-red-400 hover:text-red-600 transition-colors">
                    Supprimer
                  </button>
                </td>
              </tr>
      ...
      {modal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-md rounded p-8">
            <h2 className="font-display text-2xl font-light italic text-[#0a0a0a] mb-6">Nouveau client</h2>
